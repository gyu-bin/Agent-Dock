import type { CodexMode, StepProvider, WorkflowKind } from './types'
...
export function inferStepProvider(input: {
  ...
  const text = `${input.stepTask ?? ''} ${input.userRequest ?? ''}`.toLowerCase()
  if (/codex|repository|repo|코드베이스|프로젝트 구조/.test(text)) return 'codex'
  return 'openai'
}

export function providerForRoleKey(
  roleKey: string,
  workflow: WorkflowKind,
): { provider: StepProvider; mode?: CodexMode } {
  if (roleKey === 'approval') {
    return { provider: 'human' }
  }
  if (roleKey === 'planner' && workflow === 'BUILD') {
    return { provider: 'openai' }
  }
  if (roleKey === 'verifier') {
    return { provider: 'codex', mode: 'verify' }
  }
  if (roleKey === 'engineer') {
    return { provider: 'codex', mode: 'implement' }
  }
  if (roleKey === 'reviewer') {
    return { provider: 'codex', mode: 'review' }
  }
  if (roleKey === 'release' && workflow === 'RELEASE') {
    return { provider: 'codex', mode: 'verify' }
  }
  return { provider: 'openai' }
}
